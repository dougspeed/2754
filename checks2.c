...
if(mode==6&&samedata==0)
{printf("Error, when joining kins, can not use argument \"--same-data NO\"\n(Kinships should have been constructed from same datafile\n\n");exit(1);}
...
if(mode==23&&num_kins==1)
{printf("Warning, with argument \"--add-grm\" it is usual to supply more than one kinship file\n\n");}

if(mode==24&&num_regs>0&&num_kins>1)
{printf("Error, when subtracting region predictors must specify only one kinship matrix (not %d)\n\n", num_kins);exit(1);}

if(mode==24&&num_kins==1&&num_regs==0)
{printf("Warning, with argument \"--sub-grm\" it is usual to supply either more than one kinship files or regions to subtract\n\n");}

if(mode==24&&num_regs>0&&kindetails==0)
{printf("Error, when subtracting region predictors from a kinship matrix, can't have \"--kinship-details NO\"\n\n");exit(1);}
}
