...
if(count2==5)	//check weightsfile consistent with mapfile
{
...
prednamesb=malloc(sizeof(char*)*count);
for(j=0;j<count;j++){prednamesb[j]=malloc(sizeof(char)*100);}
read_strings(weightsfile, prednamesb, count, 5, 0);

for(j=0;j<count;j++)
{
if(strcmp(allsnps[j], prednamesb[j])!=0)
{printf("Error, %s does not align with %s; there are different predictors (%s and %s) on Row %d\n\n", weightsfile, mapfile, prednamesb[j], allsnps[j], j+1);exit(1);}
}

for(j=0;j<count;j++){free(allsnps[j]);free(prednamesb[j]);};free(prednamesb);free(allsnps);
}
...
{
int i, j, count, rowlength, gen, gen2;
...
rowbits=malloc(sizeof(unsigned char)*rowlength);
...
if(ftell(input)!=(3+rowlength*num_preds)*sizeof(char))
...
if(fseek(input, 3+rowlength*keeppreds[j], SEEK_SET)!=0)
{printf("Error in %s; unable to find Predictor %d\n\n", bedfile, j+1);exit(1);}
if(fread(rowbits, sizeof(unsigned char), rowlength, input)!=rowlength)
...
datatemp=malloc(sizeof(float)*num_samples);
...
//check the length
fseek(input, 0, SEEK_END);
if(ftell(input)!=num_samples*num_preds*sizeof(float))
...
if(fseek(input, num_samples*keeppreds[j]*sizeof(float), SEEK_SET)!=0)
{printf("Error in %s; unable to find Predictor %d\n\n", speedfile, j+1);exit(1);}
if(fread(datatemp, sizeof(float), num_samples, input)!=num_samples)
{printf("Error in %s; unable to read for Predictor %d\n\n", speedfile, j+1);exit(1);}
