...
FILE *input1, *output2, *output3, *output4, *output5, *output6;
char filename1[500], filename2[500], filename3[500], filename4[500], filename5[500], filename6[500];
...
sprintf(filename5, "%sregionALL", folder);
if((output5=fopen(filename5, "w"))==NULL)
{printf("Error opening %s\n\n", filename5);exit(1);}
sprintf(filename6, "%sregionmarks", folder);
...
for(j=gstarts[start];j<gends[end];j++){fprintf(output5,"%s\n", prednames[j]);}
fprintf(output6,"%d %d\n", start+1, end+1);
}	//end of gene g sig
}	//end of g loop
