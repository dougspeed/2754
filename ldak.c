...
//print out parameters depending on mode (do some setting as well)
#include "param.c"
...
if(num_kins>0&&(mode==9||mode==16||mode==17||(mode==24&&num_regs>0)))	//read kins
{
...
//can thin regions here - or do as separate step ???

get_stats_qc(rkeepcentres, rkeepmults, rdata, num_samples_use, rdata_length, minmaf, maxmaf, minvar, minobs, missingvalue, power, ignoremissing, rprednames, 2, rweights);

//also need to check regs ok ???
}
...
if(mode==19&&num_kins>0)	//do blup prep which required ids
{
...
get_stats_qc(keepcentres, keepmults, data, num_samples_use, data_length, minmaf, maxmaf, minvar, minobs, missingvalue, power, ignoremissing, prednames, 1, NULL);
...
weightsum+=get_stats_qc(keepcentres+bitstart, keepmults+bitstart, data, num_samples_use, bitlength, minmaf, maxmaf, minvar, minobs, missingvalue, power, ignoremissing, prednames, 2, weights+bitstart);
...
write_kins(outfile, kins, weightsum, ids1, ids2, num_samples_use, keeppreds_use, prednames, keepcentres, keepmults, weights, al1, al2, data_length, kinraw, kingz);
...
printf("Joining %d kinship matrices; saved as %s\n", num_kins, outfile);
manip_kins(outfile, num_kins, kinstems, mapfile, kinraw, kingz, 0);
free(kins);free(divs);
}
...
weightsum=get_stats_qc(keepcentres+gstarts[g]+count, keepmults+gstarts[g]+count, data+count*num_samples_use, num_samples_use, gends[g]-gstarts[g]-count, minmaf, maxmaf, minvar, minobs, missingvalue, power, ignoremissing, prednames, 2, weights+gstarts[g]+count);
...
write_kins(outfile, kins, weightsum, ids1, ids2, num_samples_use, keeppreds_use, prednames, keepcentres, keepmults, weights, al1, al2, length, kinraw, kingz);
}	//end of mode=8
...
if(mode==23||(mode==24&&num_regs==0))
{
kins=malloc(sizeof(double)*num_samples_use*num_samples_use);
divs=malloc(sizeof(double)*num_samples_use*num_samples_use);
for(i=0;i<num_samples_use*num_samples_use;i++){kins[i]=0;divs[i]=0;}

printf("Manipulating %d kinship matrices; saved as %s\n", num_kins, outfile);
if(mode==23){manip_kins(outfile, num_kins, kinstems, mapfile, kinraw, kingz, 0);}
if(mode==24){manip_kins(outfile, num_kins, kinstems, mapfile, kinraw, kingz, 1);}
free(kins);free(divs);
}
...
adjust_details(outfile, kinstems[0], rprednames, rkeepcentres, rkeepmults, rweights, rdata_length);

sub_kins();

write_kins(outfile, kins, weightsum, ids1, ids2, num_samples_use, keeppreds_use, prednames, keepcentres, keepmults, weights, al1, al2, num_preds_use, kinraw, kingz);
free(kins);free(keepcentres);free(keepmults);free(weights);
}
*/
...
if(num_kins>0&&(mode==9||mode==16||mode==17||(mode==24&&num_regs>0)))
